...
SPLITS = [
    (
        "AvaloniaSemanticBinder.TransformExtensions.cs",
        "private static ResolvedTransformExtensions BuildResolvedTransformExtensions(",
    ),
    (
        "AvaloniaSemanticBinder.NodeTypeResolution.cs",
        "private static string ResolveGeneratedClassModifier(",
    ),
    (
        "AvaloniaSemanticBinder.ObjectNodeBinding.cs",
        "private static ResolvedObjectNode BindObjectNode(",
    ),
    (
        "AvaloniaSemanticBinder.ConstructionConditions.cs",
...
def main() -> int:
    ...
    for _, anchor in SPLITS:
        ...
        if index < 0:
            ...
            if split_targets_exist:
                print("Split anchors are not present. Core binder appears to be already split; no changes applied.")
                return 0
    ...
    if anchor_indexes != sorted(anchor_indexes):

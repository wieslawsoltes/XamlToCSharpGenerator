    ...
    public XamlSourceGenRuntimeMcpServer(Stream input, Stream output)
    {
        ...
        _server = new McpServerCore(
            ...
            new McpServerInfo(
                ...
                Instructions: "Provides AXSG runtime hot reload, hot design, and studio query tools from a live process. Prefer resources/subscribe for status and event resources, and use tools for one-shot queries or operations."),
            AxsgRuntimeMcpCatalog.CreateTools(runtimeQueryService),
            AxsgRuntimeMcpCatalog.CreateResources(runtimeQueryService, _eventStore),
            new McpServerCapabilities(ResourcesSubscribe: true));
        _notificationBridge = new AxsgRuntimeMcpNotificationBridge(_server, _eventStore);
    }
